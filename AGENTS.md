...
- In-app Catalan **Política de privadesa** / **Termes d'ús**: [`web/src/lib/legalDocs.ts`](web/src/lib/legalDocs.ts) (`LEGAL_POLICY_VERSION`, currently `6 d'agost de 2026`, must match backend `ORACLE_POLICY_VERSION` / default). Not legal advice; set contact + controller name before public launch.
...
- Frontend `needsValidation`: mandatory second take unless top score ≥ 0.50 **and** top-two gap ≥ 0.15 **and** the top-two macros are geographically coherent (or the runner-up is weak &lt; 0.20); merge with `mergeValidationResults` (same top → clearer; else average). If still uncertain, optional third take. After three takes, unresolved results de-emphasize the illustrative comarca pin and show the full distribution ([`needsValidation.ts`](web/src/lib/needsValidation.ts), [`ResultsMapStage.tsx`](web/src/components/ResultsMapStage.tsx)).
